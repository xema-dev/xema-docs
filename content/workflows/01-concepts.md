...
jobs:
  ...
  review:
    ...
    uses: xema/review       # Reviewer pool over `analyze`'s outputs
```
...
- `xema/review` — Reviewer pool over one or more subjects, optional in-loop redraft (humans + agents mix)
- `xema/emit-artifact` — Save artifact to store
- `xema/decision-gate` — Pinned-version approval gate
...
Pin the live JSON Schema in your YAML editor for instant validation and autocomplete:

```yaml
# yaml-language-server: $schema=https://xema.dev/schemas/workflow/v1alpha1/Workflow.json
apiVersion: xema.dev/workflow/v1alpha1
kind: Workflow
...
```

URLs follow the Kubernetes apiVersion convention `<group>/<version>/<Kind>` so the schema URL is a deterministic function of the manifest's `apiVersion` + `kind`:

| URL | Manifest |
|---|---|
| `/schemas/workflow/v1alpha1/Workflow.json` | `apiVersion: xema.dev/workflow/v1alpha1`, `kind: Workflow` |
| `/schemas/workflow/v1alpha1/ReusableWorkflow.json` | `apiVersion: xema.dev/workflow/v1alpha1`, `kind: ReusableWorkflow` |
| `/schemas/workflow/v1alpha1/Action.json` | `apiVersion: xema.dev/workflow/v1alpha1`, `kind: Action` |

All public, cached (`cache-control: public, max-age=300`).
...
- `xema/agent` — Invoke an LLM agent
- `xema/review` — Reviewer pool with optional redraft loop
...
**Next**: Read [Features & Capabilities](./02-features.md) to understand what you can build with these concepts.
