...
The host shell exposes an xterm.js-backed terminal at `/shell` in `submodules/code-guilds-web`. The page wires xterm.js (with `@xterm/addon-fit`, `@xterm/addon-web-links`, `@xterm/addon-search`) to the `/shell/ws` gateway through the `useXemaShellSession()` hook.
